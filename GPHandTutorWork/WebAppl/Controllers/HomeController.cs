	...
	public class HomeController : Controller
	{
		...
		public IActionResult AgreementGPH_Create(int id,string subject, int group,int teacher,string datestart, string dateend,int bet, int term, string action)
		{
			...
			if (action == "Сохранить")
			{
				...
				{
					ViewBag.ErrorMessage = "Введите корректно группу или предмет или преподавателя или семестр";
					ViewBag.Role = Role;
					ViewBag.subject = APIclient.GetRequest<List<CurriculumViewModel>>("api/main/get_unique_subject").Select(s => s.Subject).Distinct().ToList();
					ViewBag.teachers = APIclient.GetRequest<List<TeacherViewModel>>("api/main/get_teachers");
					return View();
		...
		public IActionResult GPH_Reported(int teacher,string datestart, string dateend, string action)
		{
			...
			{
				ViewBag.ErrorMessage = "Введите корректно даты для выборки и выберите преподавателя из выпадающего списка ";
				var teachers = APIclient.GetRequest<List<TeacherViewModel>>("api/main/get_full_teachers");
		...
		public IActionResult WorkEmployeeReport( string datestart, string dateend, string action)
		{
			...
			if (action == "Скачать")
			{
				...
				//тут ничего не переделано под нужный отчёт
